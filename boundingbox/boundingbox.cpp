...
int main(int argc, char* argv[])
{
	...
	BITMAP* image1 = load_bitmap("image1.bmp", NULL); //첫번째 오브젝트를 위한 이미지
	BITMAP* image2 = load_bitmap("image2.bmp", NULL); //두번째 오브젝트를 위한 이미지
	...
	g_object1.posX = 0;
	g_object1.posY = 0;
	g_object1.width = image1->w;
	g_object1.height = image1->h;

	g_object2.posX = 100;
	g_object2.posY = 100;
	g_object2.width = image2->w;
	g_object2.height = image2->h;
	...
	while (!key[KEY_ESC]) 
	{
		...
		//화면을 렌더링한다.
		RenderScene(buffer, image1, image2);
	}
